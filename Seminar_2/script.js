...
// 2й вариант
// простой вариант в данной задаче был был
// add: (a, b) => a + b,
...
class Person {
  ...
  introduce = () =>
    console.log(
      `Вывод: My name is ${this.name}, and I'm ${this.age} years old`
    );
}
...
class BankAccount {
  ...
  withdraw = (amount) => {
    ...
    if (this.balance >= amount) {
      ...
      console.log(
        `Недостаточно средств на счету ${this.accountNumber}, на счету ${this.balance}`
      );
    }
  };
  displayInfo = () =>
...
sumDigits = (positiveInteger) =>
  ...
  positiveInteger < 10
    ? positiveInteger
    : (positiveInteger % 10) + sumDigits(Math.floor(positiveInteger / 10));
console.log(sumDigits(123)); // Ожидаемый результат: 6 (1 + 2 + 3)
