...
Метод speak() - выводит в консоль звук, издаваемый данным животным.
...
class ShoppingCart {
  ...
  constructor(customerName, initialTotalCost = 0) {
    ...
    this.totalCost = initialTotalCost; // начальная общая стоимость заказа (сначала 0)
    this.items = [];
  }

  addItem(product, quantity = 1) {
    // метод добавляет товар в корзину
    this.items.push({ product, quantity }); // товар и коли-во начиная с 1шт
  ...
  checkout() {
    // оформляет заказ, выводит стоимость заказа и спасибо за покупку
    console.log(
